...
class TemporalPointPillar(PointPillar):
    ...
    def _gt_occ_mask(self, batch_dict, bev_h: int, bev_w: int):
        ...
        B, M, _ = gt7.shape
        scores = torch.ones(B, M, device=gt7.device, dtype=gt7.dtype)

        mask = boxes_to_bev_masks(
            gt7,
            scores,
            bev_h,
            bev_w,
    ...
    def forward(self, batch_dict, t_seq: int = 0, det_instance_masks_prev: torch.Tensor = None, T_rel: torch.Tensor = None):
        aux_loss = None
